...
def main() -> None:
    ...
    for period_name, (start, end) in periods.items():
        for market in args.markets:
            ...
            for window, horizon, tail_prob in itertools.product(args.windows, args.horizons, args.tail_probs):
                rm = rolling_metrics_for_pivot(
                    pivot, market, period_name, window, horizon, args.step, tail_prob,
                    args.min_complete_ratio, args.min_assets_window, args.dq_grid_size
                )
                if not rm.empty:
                    all_roll.append(rm)
                tg = tail_group_portfolios(
                    pivot, market, period_name, window, horizon, args.step, tail_prob,
                    args.min_complete_ratio, args.min_assets_tail_group, args.n_tail_groups
                )
                if not tg.empty:
