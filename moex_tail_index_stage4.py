...
def main() -> None:
    ...
    if cfg.n_jobs and cfg.n_jobs > 1:
        with ProcessPoolExecutor(max_workers=cfg.n_jobs) as ex:
            futs = [ex.submit(estimate_security_windows, t) for t in tasks]
            for i, fut in enumerate(tqdm(as_completed(futs)), 1):
                try:
                    rows.extend(fut.result())
                except Exception as e:
                    print(f"      worker failed: {e}")
                if i % 25 == 0 or i == len(futs):
                    print(f"      completed {i}/{len(futs)} securities; estimates={len(rows):,}")
    else:
        for i, t in enumerate(tqdm(tasks), 1):
            rows.extend(estimate_security_windows(t))
